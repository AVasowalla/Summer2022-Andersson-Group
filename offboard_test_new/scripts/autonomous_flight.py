...
class Drone:
	...
	def get_sensor_waypoints(self):
		...
		xs1 = sensor1[0]-drone_initial[0]
		ys1 = sensor1[1]-drone_initial[1]
		zs1 = height

		xs2 = sensor2[0]-drone_initial[0]
		ys2 = sensor2[1]-drone_initial[1]
		zs2 = height

		self.waypoints = np.array([[0,0,0,1], [0,0,height,3],[xs1,ys1,zs1,3],[xs2,ys2,zs2,3], [0,0,height,3]])
	...
	def mission(self):
		...
		waypoints = self.Torelative(self.waypoints)
		print(waypoints)
		num_waypoints = waypoints.shape[0]
		while not rospy.is_shutdown():
			self.arm()
